  ...
  shared_examples 'congress' do
    ...
    context 'with default parameters' do
      ...
      it 'installs packages' do
        is_expected.to contain_package('congress').with(
          :name   => platform_params[:congress_common_package],
          :ensure => 'present',
      ...
      it 'configures rabbit' do
        ...
        is_expected.to contain_congress_config('oslo_messaging_notifications/transport_url').with_value('<SERVICE DEFAULT>')
        is_expected.to contain_congress_config('oslo_messaging_notifications/driver').with_value('<SERVICE DEFAULT>')
      end

      it 'passes purge to resource' do
        is_expected.to contain_resources('congress_config').with({
          :purge => false
        })
      end
    ...
    context 'with overridden parameters' do
      let :params do
        {
          ...
          :notification_driver                => 'ceilometer.compute.congress_notifier',
      ...
      it 'configures rabbit' do
        is_expected.to contain_congress_config('DEFAULT/rpc_backend').with_value('rabbit')
        is_expected.to contain_congress_config('DEFAULT/transport_url').with_value('rabbit://rabbit_user:password@localhost:5673')
      ...
      it 'configures various things' do
        is_expected.to contain_congress_config('oslo_messaging_notifications/transport_url').with_value('rabbit://rabbit_user:password@localhost:5673')
        is_expected.to contain_congress_config('oslo_messaging_notifications/driver').with_value('ceilometer.compute.congress_notifier')
        is_expected.to contain_congress_config('oslo_messaging_notifications/topics').with_value('openstack')
      end

      context 'with multiple notification_driver' do
        before { params.merge!( :notification_driver => ['ceilometer.compute.congress_notifier', 'congress.openstack.common.notifier.rpc_notifier']) }

        it { is_expected.to contain_congress_config('oslo_messaging_notifications/driver').with_value(
          'ceilometer.compute.congress_notifier,congress.openstack.common.notifier.rpc_notifier'
        ) }
      end
    ...
    context 'with overriden amqp parameters' do
      ...
      it 'configures amqp' do
        is_expected.to contain_congress_config('DEFAULT/rpc_backend').with_value('amqp')
        is_expected.to contain_congress_config('DEFAULT/transport_url').with_value('amqp://amqp_user:password@localhost:5672')
  ...
  on_supported_os({
    ...
    context "on #{os}" do
      ...
      let(:platform_params) do
        case facts[:osfamily]
        when 'Debian'
          { :congress_common_package => 'congress-common' }
        when 'RedHat'
          { :congress_common_package => 'openstack-congress-common' }
        end
      end
